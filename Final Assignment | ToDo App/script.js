...
const get = await getData();
  get.forEach((x) => {
    ...
    x.done === true ? input.classList.add("textDecoration") : input.classList.remove("textDecoration");
    ...
    const changedId = x._id;
    const inputField = x.description;
    completeBtn.addEventListener("click", (e) => {
      const changedData = {description: inputField, done: true};
